...
def expected_ellipsoid_norm(Sigma_norm, mu, sigma_noise, Q_1, c_1, Q_2, c_2, dim, gamma_1, gamma_2):
    ...
    mean_1 = matrix(np.zeros((dim, 1)))
    mean_2 = matrix(np.zeros((dim, 1)))
    var_1 = matrix(np.zeros((dim, dim)))
    var_2 = matrix(np.zeros((dim, dim)))
    ...
    var_1 = (var_1 - mean_1@mean_1.T/2500)/2499
    var_2 = (var_2 - mean_2@mean_2.T/2500)/2499
    mean_1 = mean_1/2500
    mean_2 = mean_2/2500
...
#     num_1_mult = 2*param["n"]*adv_queries*log(std_1_mult) + 2*param["n"]*(log(sqrt(2/3)*2**(-param["delta"])) + log(3.2*2**(-param["delta"])))
    ...
    for i in range(expr_num):
        ...
        mu_re = np.zeros((2, 4, 1))
        c_re = np.zeros((2, 1))
        mu_im = np.zeros((2, 4, 1))
        c_im = np.zeros((2, 1))
        Sigma_re_ns1 = np.zeros((2, 4, 4))
        Sigma_re_ns2 = np.zeros((2, 4, 4))
        Sigma_im_ns1 = np.zeros((2, 4, 4))
        Sigma_im_ns2 = np.zeros((2, 4, 4))
        new_Sigma_ns1 = np.zeros((2, 4, 4))
        new_Sigma_ns2 = np.zeros((2, 4, 4))
        new_Sigma_ns2_2 = np.zeros((2, 4, 4))
        new_mu_ns1 = np.zeros((2, 4, 1))
        new_mu_ns2 = np.zeros((2, 4, 1))
        new_mu_ns2_2 = np.zeros((2, 4, 1))
        Vol_re_ns1 = np.zeros((2, 1))
        Vol_re_ns2 = np.zeros((2, 1))
        Vol_im_ns1 = np.zeros((2, 1))
        Vol_im_ns2 = np.zeros((2, 1))
        ...
        for i in range(2):
            ...
            gamma_re_ns1 = (x[0]-mu_re[i]).T@Q_re[i]@(x[0]-mu_re[i]) + c_re[i] + np.random.normal(0, sqrt(param["n"]//2)*sigma_eps_1_mult, 1)   
            ...
            ellnorm = (x[0]).T@Sigma_elps_inv@(x[0])
            ...
            gamma_im_ns1 = (x[0]-mu_im[i]).T@Q_im[i]@(x[0]-mu_im[i]) + c_im[i] + np.random.normal(0, sqrt(param["n"]/2)*sigma_eps_1_mult, 1)
            # Construct approximate ellipsoids and intersect them
            ...
            gamma_re_ns2  = (x[0]-mu_re[i]).T@Q_re[i]@(x[0]-mu_re[i]) + c_re[i] + np.random.normal(0, sqrt(param["n"]/2)*std_1_mult_2, 1)
            gamma_im_ns2 = (x[0]-mu_im[i]).T@Q_im[i]@(x[0]-mu_im[i]) + c_im[i] + np.random.normal(0, sqrt(param["n"]/2)*std_1_mult_2, 1)
            # Construct approximate ellipsoids and intersect them
            mean_1, var_1, mean_2, var_2 = expected_ellipsoid_norm(Sigma_sqrt, matrix(np.zeros((4,1))), sqrt(param["n"]/2)*std_1_mult_2, Q_re[i], mu_re[i], Q_im[i], mu_im[i], 4, gamma_re_ns2 - c_re[i], gamma_im_ns2 - c_im[i])
            Sigma_1_1 = np.linalg.pinv(var_1)/4
            Sigma_2_1 = np.linalg.pinv(var_2)/4
            print("Norm based on real: ", (x[0] - mean_1).T@Sigma_1_1@(x[0] - mean_1))
            print("Norm based on im: ", (x[0] - mean_2).T@Sigma_2_1@(x[0] - mean_2))
            noint_flag = empty_ellipsoid_hyperboloid_intersection(mu1 = matrix(mean_1), Sigma1 = matrix(Sigma_1_1), mu2 = matrix(mean_2), Sigma2 = matrix(Sigma_2_1), lb = 10^(-40))                
            ellnorm1 = 0
            ...
            if noint_flag == 0:
                ...
                ellnorm1 = (x[0] - new_mu_ns2[i]).T@new_Sigma_ns2[i]@(x[0] - new_mu_ns2[i])
            else:
                if ln(matrix(Sigma_1_1).det()) > ln(matrix(Sigma_2_1).det()):
                    new_mu_ns2[i] = mean_1
                    new_Sigma_ns2[i] = matrix(Sigma_1_1)
                    ellnorm1 = (x[0] - new_mu_ns2[i]).T@new_Sigma_ns2[i]@(x[0] - new_mu_ns2[i])
                    Vol_2 = ln(matrix(new_Sigma_ns2[i]).det())
                else:
                    new_mu_ns2[i] = mean_2
                    new_Sigma_ns2[i] = matrix(Sigma_2_1)
                    ellnorm1 = (x[0] - new_mu_ns2[i]).T@new_Sigma_ns2[i]@(x[0] - new_mu_ns2[i])
                    Vol_2 = ln(matrix(new_Sigma_ns2[i]).det())
        ...
        noint_flag = empty_ellipsoid_hyperboloid_intersection(mu1 = matrix(new_mu_ns2[0]), Sigma1 = matrix(new_Sigma_ns2[0]), mu2 = matrix(new_mu_ns2[1]), Sigma2 = matrix(new_Sigma_ns2[1]), lb = 10^(-40))                
        if noint_flag == 0:
                new_mu, new_Sigma = ellipsoid_hyperboloid_intersection(mu1 = matrix(new_mu_ns2[0]), Sigma1 = matrix(new_Sigma_ns2[0]), mu2 = matrix(new_mu_ns2[1]), Sigma2 = matrix(new_Sigma_ns2[1]), lb = 10^(-40))
                Vol_2 = float(ln(matrix(new_Sigma).det()))
                ellnorm1 = (x[0] - new_mu).T@new_Sigma@(x[0] - new_mu)
                if ln(matrix(new_Sigma_ns2[0]).det()) > ln(matrix(new_Sigma_ns2[1]).det()) and Vol_2 < ln(matrix(new_Sigma_ns2[0]).det()):
                    Vol_2 = ln(matrix(new_Sigma_ns2[0]).det())
                elif ln(matrix(new_Sigma_ns2[1]).det()) > ln(matrix(new_Sigma_ns2[0]).det()) and Vol_2 < ln(matrix(new_Sigma_ns2[1]).det()):
                    Vol_2 = ln(matrix(new_Sigma_ns2[1]).det())
        else:
            if ln(matrix(new_Sigma_ns2[0]).det()) > ln(matrix(new_Sigma_ns2[1]).det()):
                new_mu = matrix(new_mu_ns2[0])
                new_Sigma = matrix(new_Sigma_ns2[0])
                ellnorm1 = (x[0] - new_mu).T@new_Sigma@(x[0] - new_mu)
                Vol_2 = float(ln(matrix(new_Sigma).det()))
            else:
                new_mu = matrix(new_mu_ns2[1])
                new_Sigma = matrix(new_Sigma_ns2[1])
                ellnorm1 = (x[0] - new_mu).T@new_Sigma@(x[0] - new_mu)
                Vol_2 = float(ln(matrix(new_Sigma).det()))
        print("Ellipsoid norm Int--Second level: ", ellnorm1)
        print("Vol Int--Second level: ", Vol_2)
