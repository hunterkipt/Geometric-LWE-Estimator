...
def intersect_SCA_ellipsoid(dbdd_int, mu_SCA, S_SCA, guess_dict, sorted_guesses, all_coordinates_attack, nointersection, knownscaling):
    """
    ...
    mu_int = matrix(mu_baseline)
    for j in range(len(intersect_indices)):
        logging("   HERE3   ", style="HEADER")
        logging("Old sigma: %3.6f"% S_baseline[j][j])
        try:
            if nointersection:
                logging("   HERE1   ", style="HEADER")
            else:
                mu_int_prime, S_int_prime = ellipsoid_intersection(matrix([mu_decfail[0,j]]), matrix([S_decfail[j,j]]), matrix([mu_baseline[0,j]]), matrix([S_baseline[j,j]]), tolerance=1.48e-08) #<=1
                mu_int[0,j] = mu_int_prime[0,0]
                S_int[j,j] = S_int_prime[0,0]
                logging("   HERE2   ", style="HEADER")
                logging("New sigma: %3.6f"% S_int[j,j])
                logging("Old sigma: %3.6f"% S_baseline[j,j])
        except ValueError:
        ...
        if error_flag > 0:
            error_flag = 0
            if (sqrt(S_baseline[j,j]) + sqrt(S_decfail[j,j])) < abs(mu_baseline[0,j] - mu_decfail[0,j]):
                S_int[j,j] = S_baseline[j,j]
                mu_int[0,j] = mu_baseline[0,j]
            else:
                sigma_prime = (abs(mu_baseline[0,j] - mu_decfail[0,j]) - sqrt(S_baseline[j,j]))**2 + .05
                try:
                    mu_int_prime, S_int_prime = ellipsoid_intersection(matrix([mu_decfail[0,j]]), matrix([sigma_prime]), matrix([mu_baseline[0,j]]), matrix([S_baseline[j,j]]), tolerance=1.48e-08) #<=1
                    mu_int[0,j] = mu_int_prime[0,0]
                    S_int[j,j] = S_int_prime[0,0]
                    logging("   HERE4   ", style="HEADER")
                    logging("New sigma: %3.6f"% S_int[j,j])
                    logging("Old sigma baseline: %3.6f"% S_baseline[j,j])
                    logging("Old sigma decfail: %3.6f"% S_decfail[j,j])
                    logging("Sigma prime: %3.6f"% sigma_prime)
                except ValueError:
                    S_int[j,j] = S_baseline[j,j]
                    mu_int[0,j] = mu_baseline[0,j]
	...
	# removed scaling by /(n)
    mu_int, S_int = extend_ellipsoid(dbdd_int, mu_int, S_int)
...
def process_before_intersection(dbdd_int, mu_SCA, S_SCA, guess_dict, sorted_guesses, all_coordinates_attack, nointersection, knownscaling, n, m):
    ...
    #if not nointersection:
    if knownscaling:
	#		#added /len(intersect_indices)
        S_decfail = S_decfail #*(decfail_norm_p/len(intersect_indices))  
        S_baseline = S_baseline #*(baseline_norm_p/len(intersect_indices))  
    else:
...
def sample_df_ellipsoid(dbdd_int, mu_SCA, S_SCA, nointersection, knownscaling, intersect_indices, n, m, coordinates_guessed, S_decfail_full, full_mu_SCA, full_S_SCA, secret):
    ...
    ellipsoid_norm_decfail = matrix([[n+1]])
    #S_SCA = S_SCA*(n) Dnote: commented out.
    while ellipsoid_norm_decfail[0,0] > n:
        mu_decfail_full = matrix(dbdd_int.u[0,n:n+m])
		#Dnote: removed /(n)
        random_vector = rng.multivariate_normal([0]*n, S_decfail_full)
        ...
        # Ensure that the DF ellipsoid norm is below n Dnote: changed to n
        ...
        if len(intersect_indices) > 0 and verbose:
           logging("DF norm: %1.10f, just P: %1.10f"%(ellipsoid_norm_decfail[0,0], decfail_norm_p))
    ...
    for i in range(n):
        if coordinates_guessed[0][i+m] != 1:
            if S_SCA[0,j] > S_decfail_full[i,i]:    
                ...
                full_S_SCA[0,i] = S_decfail_full[i,i]
...
def process_after_intersection(dbdd_int, all_coordinates_attack, nointersection, knownscaling, intersect_indices, n, m, guesses, coordinates_guessed, full_mu_SCA, full_S_SCA, baseline_norm_p, decfail_norm_p, S_baseline, mu_baseline, S_decfail, mu_decfail, S_baseline_n_g, mu_baseline_n_g, S_int, mu_int):
    ...
    if knownscaling:
		#added scaling by (len(intersect_indices)/baseline_norm_p)
        S_int = S_int #*(len(intersect_indices)/baseline_norm_p)  
        S_decfail = S_decfail #*(len(intersect_indices)/decfail_norm_p)   
        S_baseline = S_baseline #*(len(intersect_indices)/baseline_norm_p)   
    ...
    S_baseline2 = diagonal_matrix(S_baseline2[0])   
...
def print_norm_det_data(nointersection, knownscaling, intersect_indices, n, m, guesses, coordinates_guessed, S_decfail_full, mu_decfail_full, secret, S_baseline, mu_baseline, S_decfail, mu_decfail, S_int, mu_int):
    ...
    elif ellipsoid_norm_int[0][0] > n or ellipsoid_norm_decfail[0][0] > n or ellipsoid_norm_baseline[0][0] > n:
		#changed above to n
...
def estimate_SCA(dbdd, measured, frodo_distribution):
    ...
    # The baseline attack
    _, dbdd_baseline, baseline_volumes = intersect_SCA_ellipsoid(dbdd_baseline, mu_SCA[0,n:n+m], S_SCA[0,n:n+m], guess_dict, sorted_guesses, True, True, True)
    ...
    return (np.array([SCA_beta]), SCA_volumes, SCA_guess_data), (np.array([baseline_beta]), baseline_volumes, baseline_guess_data), (np.array([known_all_coords_beta]), known_all_coords_volumes, known_all_coords_guess_data, valid_known_all_coords) 
...
def one_experiment(id, aargs):
    ...
    SCA_ellipsoid_norm = matrix([[n+1]])
    while SCA_ellipsoid_norm[0][0] > n:
        ...
        S_SCA = S_SCA[0,n:n+m] #took out scaling by n
    ...
    results_SCA, results_baseline, results_known_all_coords = estimate_SCA(dbdd, measured, frodo_distribution)
    ...
    if verbose:
        ...
        logging("All Coordinates Attack (known norm):   %3.2f bikz %3.2f bikz %4d %0.2f, volume: %3.2f" % (results_known_all_coords[2][0], results_known_all_coords[0][0], results_known_all_coords[2][2], results_known_all_coords[2][1], results_known_all_coords[1][2]), style="HEADER")

    return results_SCA, results_baseline, results_known_all_coords
...
def run_experiment():
    ...
    for params in ['CCS1']:#'CCS1']:#, 'CCS2', 'CCS3', 'CCS4', 'NIST1', 'NIST2']:
        ...
        for r in res:
            ...
            logging("All Coordinates Attack (known norm):   %3.2f bikz %3.2f bikz %4d %0.2f, volumes: %3.2f" % (r[2][2][0], r[2][0][0], r[2][2][2], r[2][2][1], r[2][1][2]), style="HEADER")
        ...
        for i in range(3):
            for j in range(result_ranges[i]):
                ...
                results_known_all_coords[i,j] = sum([r[2][i][j] for r in res])/nb_tests_per_params
